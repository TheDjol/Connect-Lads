...
GamePadState Xbox360Controller2::update()
{
	...
	if (isConnected())	//Checks if the controller is connected
	{
		...
		if (sf::Joystick::isButtonPressed(0, 1))		//These if statements check if a button is pressed
			m_currentState.B = true;					//And set it to be pressed if it is
		else                                            //If theyre not pressed
			m_currentState.B = false;					//Sets them to not be pressed
		...
		m_currentState.triggers = sf::Joystick::getAxisPosition(0, sf::Joystick::Z);	//Gets the triggers axis
		...
		m_currentState.LeftThumbStick.x = sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::X);		//Gets the joysticks positions
